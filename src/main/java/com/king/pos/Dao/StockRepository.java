...
public interface StockRepository extends JpaRepository<StockProduit, Long> {
    ...
    @Query("""
        select sp
        from StockProduit sp
        join fetch sp.produit p
        left join fetch p.categorie
        left join fetch sp.depot
        where p.actif = true
    """)
    List<StockProduit> findAllAvecProduitCategorieEtDepot();
    ...
    @Query("""
        ...
        left join fetch sp.depot
        where p.actif = true
          and sp.quantiteDisponible > 0
    """)
    List<StockProduit> findAllDisponiblesAvecProduit();
